# 📊 Classifying Cryptocurrencies by Market Capitalization

[![Streamlit App](https://static.streamlit.io/badges/streamlit_badge_black_white.svg)](https://classifying-cryptocurrencies-by-market-capitalization-bjyn9rn4.streamlit.app/)

## 🔗 Live Dashboard
**Main Deployment:** [Interactive Crypto Analysis Dashboard](https://classifying-cryptocurrencies-by-market-capitalization-bjyn9rn4.streamlit.app/)

---

## 📌 Project Overview
This repository features a comprehensive data analysis of the 2018 cryptocurrency market peak. The project focuses on wealth distribution (Market Cap), volatility trends (24h/7d), and asset tiering to provide a granular view of market dynamics during high-speculation periods.

## 🛠️ Technology Stack & Tools

| Category | Technologies |
| :--- | :--- |
| **Core Language** | ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) |
| **Data Analysis** | ![Pandas](https://img.shields.io/badge/pandas-%23150458.svg?style=for-the-badge&logo=pandas&logoColor=white) ![NumPy](https://img.shields.io/badge/numpy-%23013243.svg?style=for-the-badge&logo=numpy&logoColor=white) |
| **Visualization** | ![Matplotlib](https://img.shields.io/badge/Matplotlib-%23ffffff.svg?style=for-the-badge&logo=Matplotlib&logoColor=black) |
| **Deployment** | ![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=Streamlit&logoColor=white) |
| **Environment** | ![Google Colab](https://img.shields.io/badge/Colab-F9AB00?style=for-the-badge&logo=googlecolab&logoColor=white) |

## 📈 Analysis Insights

### 1️⃣ Market Capitalization Dominance
Analysis of the 'Top 10' assets reveals the significant concentration of market value, primarily driven by Bitcoin's dominance over alternative coins.
![Market Cap](images/fig1_marketcap.png)

### 2️⃣ Short-Term & Weekly Volatility
Comparative analysis of price fluctuations over 24-hour and 7-day windows to distinguish between temporary noise and sustained market trends.
![24h Volatility](images/fig2_24h.png)

### 3️⃣ Market Segmentation
Cryptocurrencies are classified into three distinct categories based on risk and size:
*   **Biggish**: Established projects with > $300M Market Cap.
*   **Micro**: Mid-tier projects between $50M - $300M.
*   **Nano**: Highly speculative small-cap projects < $50M.
![Classification](images/fig4_classification.png)

---

## 👩‍💻 Author
**M. Divya Lalitha**
*   **GitHub**: [M-Divya29](https://github.com/M-Divya29)

